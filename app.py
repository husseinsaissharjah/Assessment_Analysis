...
with tab1:
    ...
    if up_file:
        ...
        # --- Read full data (Row 2 = headers) ---
        raw = pd.read_excel(up_file, header=1)
        obj_names = [col for col in raw.columns if col != 'Student Name']

        # Find the 'Points for Objectives' row
        mask = raw.iloc[:, 0].astype(str).str.contains("Points for Objectives", case=False, na=False)
        if not mask.any():
            st.error("❌ Row 3 must have 'Points for Objectives' in the first column with max marks next to it.")
            st.stop()
        ...
        # Student data = everything except that row
        student_df = raw[~mask].copy()
        student_df = student_df.dropna(subset=['Student Name'])
        for col in obj_names:
            student_df[col] = pd.to_numeric(student_df[col], errors='coerce').fillna(0)

        total_max = sum(obj_max)
        st.info(f"📋 Auto Total Max Mark = **{total_max}** (from Excel)")
        ...
        for _, row in student_df.iterrows():
            for j, col in enumerate(obj_names):
                ...
                mx = obj_max[j]
                if mark > mx:
                    errors.append(f"• {row['Student Name']}: '{col}' mark = {mark} exceeds max {mx}")
                if mark < 0:
                    errors.append(f"• {row['Student Name']}: '{col}' mark = {mark} is negative")
        ...
        if errors:
            st.error("🚫 Data entry problems found. Please fix in Excel and re-upload:\n" + "\n".join(errors))
            analyze_disabled = True
        else:
            st.success("✅ No data entry errors found. You can analyze!")

        if st.button("🔍 Analyze Assessment", disabled=analyze_disabled):
            results = []
            for _, row in student_df.iterrows():
            ...
            try:
                ...
                pdf.cell(0, 10, f"Overall: {overall}", ln=True)
                ...
                e2.error(f"PDF export issue: {ex}")
