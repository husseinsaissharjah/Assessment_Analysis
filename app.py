...
def read_meta_and_pct(f):
    """Returns (meta_dict, df_with_Pct_column) or (None,None) if format wrong"""
...
# ================= TAB 2 (with metadata + dynamic title) =================
with tab2:
    ...
    if all(files):
        ...
        merged['Difference'] = (merged[pct_cols[-1]] - merged[pct_cols[0]]).round(1) if False else (merged[pct_cols[-1]] - merged[pct_cols[0]]).round(1)
...
with tab3:
    ...
    if f1 and f2:
        ...
        m1c.metric("🟩 Growth", gc); m2c.metric("🟥 Decay", dc); m3c.metric("🟨 Same", sc)
