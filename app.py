...
/* Secondary buttons: grey always, even when focused/clicked */
...
.stSidebar button[data-testid="stBaseButton-secondary"]:focus-visible {
    background-color: #f0f2f6 !important !important;
    color: #333 !important;
    box-shadow: none !important;
    outline: none !important;
}

/* Primary button (active page): blue always */
...
if page == "🏠 Home":
    ...
    if f1 and f2:
        ...
        st.subheader("📊 Comparison Table (Percentage Based)")
        st.dataframe(merged.style.map(color_cell, subset=['Status']), use_container_width=True)
        cnt = merged['Status'].value_counts().to_dict? No
        cnt = merged['Status'].value_counts().to_dict()
        gc = cnt.get('Growth', 0); dc = cnt.get('Decay', 0); sc = cnt.get('Same', 0)
        st.subheader("📢 Summary")
