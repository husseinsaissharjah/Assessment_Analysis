...
if page == "🏠 Home":
    ...
    if map_file:
        try:
            ...
            assass = map_df.copy()
            assass["Previous RIT"] = pd.to_numeric(assass["Previous RIT"], errors="coerce")
            assass["Current RIT"] = pd.to_numeric(assass["Current RIT"], errors="coerce")
            assass["Percentile"] = pd.to_numeric(assass["Percentile"], errors="coerce")
            assass["RIT Growth"] = assass["Current RIT"] - assass["Previous RIT"]
            assass["Growth Status"] = assass["RIT Growth"].apply(lambda x: "Growth" if x > 0 else "Decay" if x < 0 else "Same")
            assass["Support Level"] = assass["Percentile"].apply(lambda x: "Intervention" if x < 25 else "Monitor" if x < 50 else "On Track" if x < 75 else "Enrichment")
            ...
            st.markdown("---")
            status_count = assass["Growth Status"].value_counts().reset_index(); status_count.columns = ["Status", "Count"]
            v1, v2 = st.columns(2)
            with v1:
                st.subheader("📈 Student Growth")
                st.plotly_chart(px.bar(assass, x="Student Name", y="RIT Growth", color="Growth Status"), use_container_width=True)
            with v2:
            ...
            st.plotly_chart(px.bar(assass, x="Student Name", y="Percentile", color="Support Level", range_y=[0, 100]), use_container_width=True)
            st.subheader("👥 Support Groups")
            support_count = assass["Support Level"].value_counts().reset_index(); support_count.columns = ["Support Level", "Students"]
            st.dataframe(support_count, use_container_width=True)
            st.subheader("📋 Student MAP Analysis")
            st.dataframe(assass.style.map(color_cell, subset=["Growth Status"]), use_container_width=True)
            map_buffer = io.BytesIO(); assass.to_excel(map_buffer, index=False)
