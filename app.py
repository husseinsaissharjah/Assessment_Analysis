...
/* Base secondary button state (Inactive pages) */
div[data-testid="stSidebar"] button[data-testid="stBaseButton-secondary"] {
    background-color: transparent !important;
    color: inherit !important;
}

/* Prevent focus/active states from staying blue on secondary buttons */
div[data-testid="stSidebar"] button[data-testid="stBaseButton-secondary"]:focus,
div[data-testid="stSidebar"] button[data-testid="stBaseButton-secondary"]:focus-visible,
...
/* Base primary button state (Active page) */
div[data-testid="stSidebar"] button[data-testid="stBaseButton-primary"] {
    background-color: #1f77b4 !important;
    color: white !important;
}

/* Ensure active page stays blue even when focus moves away */
div[data-testid="stSidebar"] button[data-testid="stBaseButton-primary"]:focus,
div[data-testid="stSidebar"] button[data-testid="stBaseButton-primary"]:focus-visible,
div[data-testid="stSidebar"] button[data-testid="stBaseButton-primary"]:active {
    background-color: #1f77b4 !important;
    color: white !important;
    box-shadow: none !important;
    outline: none !important;
...
def read_objectives_file(f):
  ...
  mask = (
      df.iloc[:, 0]
      .astype(str)
      .str.contains("Points for Objectives", case=False, na=False)
  )
  if not mask.any():
    return None, None
  ...
  df["Pct"] = (
      (df["Obtained"] / total_max * 100).round(1) if total_max else 0.0
  )
  return meta, df
...
def read_total_file(f):
  ...
  data = data[
      data.iloc[:, 0].astype(str).str.lower().str.contains("total") == False
  ]
  ...
  data["Pct"] = (
      (data[total_col] / max_total * 100).round(1) if max_total else 0.0
  )
  return meta, data
...
if page == "🏠 Home":
  ...
  st.markdown(
      "Analyze MAP, internal assessments, grades, and student performance in"
      " seconds."
  )
  st.markdown("---")
  ...
  st.markdown(
      "The SAIS Analyzer is designed to help teachers, coordinators, and"
      " school leaders analyze student achievement quickly and consistently."
  )
  st.markdown("---")
  c1, c2, c3 = st.columns(3)
  with c1:
    ...
    st.write(
        "Analyze one assessment at a time using learning objectives and"
        " student marks."
    )
    st.markdown(
  ...
  with c2:
    ...
    st.write(
        "Compare multiple assessments for the same group of students and"
        " monitor their academic progress."
    )
    st.markdown(
        "- Compare assessment results.\n- Convert scores to percentages.\n-"
        " Identify student growth.\n- Identify student decay.\n- Identify"
        " students with stable performance.\n- View grade performance trends."
    )
  with c3:
    st.subheader("🎯 Achievement & Gaps")
    st.write(
        "Compare Internal Assessment results with External Assessment results"
  ...
  st.markdown(
      "The MAP Analysis service helps analyze student performance using MAP"
      " Growth RIT scores."
  )
  st.info(
      "**What is a RIT Score?**\n\nA RIT score is the scale used in MAP Growth"
      " to measure a student's academic achievement and instructional level."
  )

elif page == "👨‍🎓 Student Analysis":
  st.header("👨‍🎓 Student Analysis")
  st.markdown(
      "Analyze a single assessment based on learning objectives and student"
      " marks."
  )
  st.download_button(
  ...
  st.info(
      ...
      " Objective Descriptions\nRow 4: 'Points for Objectives' + Maximum"
      " Marks\nRow 5+: Student Marks\nLeave empty or enter 'A' for absent"
      " students."
  )
  ...
  if up_file:
    ...
    st.markdown(
        f"### 📝 Name: **{meta_info.get('Assessment name', 'N/A')}** | 📚"
        f" Subject: **{meta_info.get('Subject', 'N/A')}**"
    )
    ...
    for _, row in student_df.iterrows():
      ...
      for j, c in enumerate(obj_names):
        if row[c] > obj_max[j]:
          errors.append(
              f"• {row['Student Name']}: {c}={row[c]} > max {obj_max[j]}"
          )
        if row[c] < 0:
          errors.append(f"• {row['Student Name']}: {c}={row[c]} negative")
      ...
      if st.button("Analyze Assessment"):
        ...
        for _, row in student_df.iterrows():
          ...
          for j, c in enumerate(obj_names):
            mk = float(row[c])
            tot += mk
            ps.append((mk / obj_max[j]) * 100 if obj_max[j] else 0)
          tp = sum(ps) / len(ps)
          lvl = (
              "Fail"
              if tp < 60
        ...
        cdf = rdf["Level"].value_counts().reset_index()
        cdf.columns = ["Level", "Count"]
        cdf["Level"] = pd.Categorical(
            cdf["Level"], categories=ORDER, ordered=True
        )
        ...
        support_count = (
            rdf["Support Level"].value_counts().reset_index()
        )
  ...
  st.info(
      "Choose the number of assessments. Upload files using the same Excel"
      " format as Student Analysis. Each assessment score will automatically"
      " be converted to a percentage before comparison."
  )
  ...
  for i in range(int(n_assess)):
    files.append(
        st.file_uploader(
            f"📄 Assessment {i + 1}", type=["xlsx", "xls"], key=f"up{i}"
        )
    )
  if all(files):
    metas = []
    ...
    for i, f in enumerate(files):
      ...
      if meta is None:
        st.error(f"❌ File {i + 1} missing 'Points for Objectives' row.")
        st.stop()
      f.seek(0)
      ...
      merged = (
          keep
          if merged is None
          else pd.merge(merged, keep, on="Student Name", how="outer")
      )
      pct_cols.append(col)
    st.subheader("📋 Assessment Information")
    for i, m in enumerate(metas):
      st.markdown(
          f"**File {i + 1} ({m.get('Assessment name', 'N/A')}):** 👩‍🏫"
          f" {m.get('Teacher Name', 'N/A')} | 🏫 {m.get('Class', 'N/A')} | 📅"
          f" {m.get('Date', 'N/A')} | 📚 {m.get('Subject', 'N/A')}"
      )
      st.markdown("**📚 Objectives:**")
      for c, d in descriptions[i].items():
        st.markdown(f"- **{c}** – {d}")
    st.markdown(
        f"### 📊 Comparing: **{' / '.join(names)}** | 📚 Subject:"
    ...
    m3.metric("🟨 Same", sc)
    cd = pd.DataFrame(
        {"Status": ["Growth", "Decay", "Same"], "Count": [gc, dc, sc]}
    )
    cd["Status"] = pd.Categorical(
        cd["Status"], categories=["Decay", "Same", "Growth"], ordered=True
    )
    v1, v2 = st.columns(2)
    with v1:
    ...
    with v2:
      ...
      pf.update_traces(textinfo="percent+label")
    ...
    st.plotly_chart(
        px.line(avg, x="Assessment", y="Average", markers=True),
        use_container_width=True,
    )
    st.subheader("📈 Student Growth (Difference)")
    st.plotly_chart(
        px.bar(merged, x="Student Name", y="Difference", color="Status"),
        use_container_width=True,
    )
    support_count = (
        merged["Support Level"].value_counts().reset_index()
    )
    support_count.columns = ["Support Level", "Students"]
    st.subheader("👥 Support Groups")
    st.dataframe(support_count, use_container_width=True)
    ...
    st.download_button(
        "📊 Download Comparison Excel", bufc.getvalue(), "Comparison.xlsx"
    )

elif page == "📈 MAP Analysis":
  st.title("📈 MAP Analysis")
  st.info(
      "### What is a RIT Score?\nThe RIT score is the scale used by MAP Growth"
      " to measure student achievement and instructional level."
  )
  st.markdown(
      "### What this analysis does\n- Calculates RIT growth.\n- Identifies"
      " Growth, Decay, or Same performance.\n- Shows student percentile.\n-"
      " Calculates grade average RIT.\n- Identifies students below the"
      " selected percentile.\n- Identifies students requiring intervention or"
      " enrichment."
  )
  st.download_button(
  ...
  map_file = st.file_uploader(
      "📄 Upload MAP Data Excel", type=["xlsx", "xls"], key="map"
  )
  if map_file:
    try:
      ...
      if missing:
        ...
        st.stop()
      assass = map_df.copy()
      assass["Previous RIT"] = pd.to_numeric(
          assass["Previous RIT"], errors="coerce"
      )
      assass["Current RIT"] = pd.to_numeric(
          assass["Current RIT"], errors="coerce"
      )
      assass["Percentile"] = pd.to_numeric(
          assass["Percentile"], errors="coerce"
      )
      assass["RIT Growth"] = assass["Current RIT"] - assass["Previous RIT"]
      assass["Growth Status"] = assass["RIT Growth"].apply(
          lambda x: "Growth" if x > 0 else "Decay" if x < 0 else "Same"
      )
      assass["Support Level"] = assass["Percentile"].apply(
          lambda x: (
              "Intervention"
              if x < 25
              else "Monitor"
      ...
      st.dataframe(assass, use_container_width=True)
      st.markdown("---")
      st.subheader("📊 MAP Summary")
      total_students = len(assass)
      avg_previous = assass["Previous RIT"].mean()
      avg_current = assass["Current RIT"].mean()
      avg_growth = assass["RIT Growth"].mean()
      avg_percentile = assass["Percentile"].mean()
      ...
      st.markdown("---")
      status_count = assass["Growth Status"].value_counts().reset_index()
      ...
      with v1:
        ...
        st.plotly_chart(
            px.bar(
                assass, x="Student Name", y="RIT Growth", color="Growth Status"
            ),
            use_container_width=True,
        )
      with v2:
        ...
        st.plotly_chart(
            px.pie(status_count, names="Status", values="Count", hole=0.3),
            use_container_width=True,
        )
      st.markdown("---")
      ...
      st.subheader("👥 Support Groups")
      support_count = assass["Support Level"].value_counts().reset_index()
      support_count.columns = ["Support Level", "Students"]
      st.dataframe(support_count, use_container_width=True)
      st.subheader("📋 Student MAP Analysis")
      st.dataframe(
          assass.style.map(color_cell, subset=["Growth Status"]),
          use_container_width=True,
      )
      map_buffer = io.BytesIO()
      assass.to_excel(map_buffer, index=False)
      st.download_button(
  ...
  f1 = st.file_uploader(
      "📄 Internal Assessment", type=["xlsx", "xls"], key="intf"
  )
  f2 = st.file_uploader(
      "📄 External Assessment", type=["xlsx", "xls"], key="extf"
  )
  if f1 and f2:
    ...
    st.subheader("📋 Assessment Information")
    st.markdown(
        f"**Internal:** 👩‍🏫 {m1.get('Teacher Name', 'N/A')} | 🏫"
        f" {m1.get('Class', 'N/A')} | 📅 {m1.get('Date', 'N/A')} | 📝"
        f" {m1.get('Assessment name', 'N/A')} | 📚 {m1.get('Subject', 'N/A')}"
    )
    st.markdown(
        f"**External:** 👩‍🏫 {m2.get('Teacher Name', 'N/A')} | 🏫"
        f" {m2.get('Class', 'N/A')} | 📅 {m2.get('Date', 'N/A')} | 📝"
        f" {m2.get('Assessment name', 'N/A')} | 📚 {m2.get('Subject', 'N/A')}"
    )
    st.markdown(
        f"### 📊 Comparing: **{m1.get('Assessment name', 'Internal')} /"
        f" {m2.get('Assessment name', 'External')}** | 📚 Subject:"
  ...
  st.title("📑 Reports")
  st.info(
      "Reports generated from each analysis can be downloaded directly from"
      " the relevant analysis page.\n\nFuture versions can include:\n•"
