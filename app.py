...
def support_level(pct):
    ...
    return (
        "Intervention" if p < 25
        else "Monitor" if p < 50
        else "On Track" if p < 75
        else "Enrichment"
    )
...
def read_objectives_file(f):
    ...
    df = df.rename(
        columns={df.columns[0]: 'Student Name'}
    )
    ...
    df['Pct'] = (
        (df['Obtained'] / total_max * 100).round(1)
        if total_max
        else 0.0
    )
...
def read_total_file(f):
    ...
    data['Pct'] = (
        (data[total_col] / max_total * 100).round(1)
        if max_total
        else 0.0
    )
...
# =========================================================
    ...
    st.markdown(
        "The SAIS Analyzer is designed to help teachers, coordinators, "
    ...
    with c2:
        ...
        st.write(
            "Compare multiple assessments for the same group of students "
    ...
    with c3:
        ...
        st.write(
            "Compare Internal Assessment results with External Assessment "
            "results to identify achievement gaps."
        )
    ...
    st.markdown(
        "The MAP Analysis service helps analyze student performance "
        "using MAP Growth RIT scores."
    )

    st.info(
        "**What is a RIT Score?**\n\n"
        "A RIT score is the scale used in MAP Growth to measure a student's "
        "academic achievement and instructional level."
    )
        ...
        if errors:
            ...
            if st.button("Analyze Assessment"):
                ...
                with v2:
                    ...
                    # FIXED: duplicate color='Level' removed
                    st.plotly_chart(
                        px.pie(
                            cdf,
                            names='Level',
...
# =========================================================
    ...
    st.info(
        ...
        "Upload files using the same Excel format as Student Analysis. "
        "Each assessment score will automatically be converted to a percentage before comparison."
    )
...
# =========================================================
    ...
    st.info(
        "Reports generated from each analysis can be downloaded directly "
        "from the relevant analysis page.\n\n"
