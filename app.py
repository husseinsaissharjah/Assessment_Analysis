...
# =========================================================
    ...
    if service == t("Compare between sections"):
        ...
        comp_type = st.radio(t("Comparison Type"), [
        ...
        if comp_type == t("By Assessment Objectives"):
            st.subheader(t("📚 By Assessment Objectives"))
            st.info(t("Select number of classes, then upload one Objective Analysis Excel file per class. Bands: Below 60% (Weak), 60-75% (Acceptable), 76-85% (Very Good), 86-100% (Excellent)."))
            ...
            if all(sec_files):
                ...
                band_order = [t("Below 60% (Weak)"), t("60-75% (Acceptable)"), t("76-85% (Very Good)"), t("86-100% (Excellent)")]
                ...
                st.subheader(t("📈 Dumbbell Chart (Class gap per Band)"))
                fig = go.Figure()
                ...
                fig.update_layout(xaxis_title=t("Student Count"), yaxis_title=t("Performance Band"), height=400)
                st.plotly_chart(fig, use_container_width=True)

                st.subheader(t("🏆 Class Order per Objective (Rank 1 = Highest Average %)"))
                rank_rows = []
        ...
        elif comp_type == t("By Assessment Total Mark"):
            st.subheader(t("📊 By Assessment Total Mark"))
            st.info(
            ...
            n_sec = st.number_input(t("Number of classes"), min_value=2, max_value=10, value=2, step=1, key="nsec_total")
            sec_files = []
            for i in range(int(n_sec)):
        ...
        elif comp_type == t("By External Benchmark Assessment"):
            st.subheader(t("🏢 By External Benchmark Assessment"))
            st.info(
                "Upload External Benchmark (objectives or total) sheets per class. "
                "All will be converted to percentage. Bands: Below 60% (Weak), 60-75% (Acceptable), "
                "76-85% (Very Good), 86-100% (Excellent)."
            )
            n_sec = st.number_input(t("Number of classes"), min_value=2, max_value=10, value=2, step=1, key="nsec_ext")
            sec_files = []
            for i in range(int(n_sec)):
