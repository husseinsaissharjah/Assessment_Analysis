...
def read_objectives_file(file):
    try:
        ...
        mask = (
            df[first_col].astype(str).str.strip().str.lower()
        ...
        for column in df.columns:
            ...
            if header == "" or header.lower() == "nan" or header.startswith("Unnamed"):
                continue
        ...
        df = df[~mask].copy()
        df = df.rename(columns={first_col: "Student Name"})
        ...
        df["Obtained"] = df[valid_cols].sum(axis=1)
        df["Pct"] = (df["Obtained"] / total_max * 100).round(1)
...
def read_total_file(file):
    try:
        ...
        data = data[
            ~data.iloc[:, 0].astype(str).str.lower().str.contains("total", na=False)
        ]

        data = data.rename(columns={data.columns[0]: "Student Name"})

        total_columns = [c for c in data.columns if "total" in str(c).lower()]
        if not total_columns:
            return None, None

        total_column = total_columns[0]
        data[total_column] = pd.to_numeric(data[total_column], errors="coerce").fillna(0)
        data["Pct"] = (data[total_column] / max_total * 100).round(1)
...
def read_gaps_file(file):
    try:
        ...
        try:
            max_total = float(raw.iloc[over_idx, 1])
        except (ValueError, TypeError):
            max_total = 100.0
        ...
        data = data[
            ~data.iloc[:, 0].astype(str).str.lower().str.contains("over", na=False)
        ]

        data = data.rename(columns={data.columns[0]: "Student Name"})

        internal_columns = [c for c in data.columns if "total of internal" in str(c).lower()]
        map_columns = [c for c in data.columns if "percentile of map" in str(c).lower()]
        ...
        data[internal_column] = pd.to_numeric(data[internal_column], errors="coerce")
        data[map_column] = pd.to_numeric(data[map_column], errors="coerce")

        data["Pct1"] = (data[internal_column] / max_total * 100).round(1)
...
def read_section_file(file):
    ...
    try:
        file.seek(0)
        raw_full = pd.read_excel(file, header=1)

        first_value = str(raw_full.iloc[0, 0]).strip().lower()
        ...
        points_mask = (
            raw_full.iloc[:, 0].astype(str).str.strip().str.lower()
            == "points for objectives"
        )
        ...
        for column in obj_names:
            try:
                obj_max[column] = float(max_row[column])
            except (ValueError, TypeError):
                obj_max[column] = 0

            if desc_row is not None:
                description = str(desc_row[column]).strip()
...
if page == "🏠 Home":
    ...
    st.markdown(f"### {t('Student Assessment & Achievement Dashboard')}")
    ...
    with c1:
        st.markdown(f"### {t('① Upload Data')}")
        st.write(t("Upload your Excel files with student marks."))

    with c2:
        st.markdown(f"### {t('② Choose Analysis')}")
        st.write(t("Pick the analysis type from the sidebar."))

    with c3:
        st.markdown(f"### {t('③ View Insights')}")
        st.write(t("See charts, gaps, and download reports."))
...
# =========================================================
    ...
    up_file = st.file_uploader(t("Upload Excel"), type=["xlsx", "xls"], key="single_objective_file")

    if up_file:
        meta, parsed_df = read_objectives_file(up_file)

        if meta is None or parsed_df is None:
            st.error(t("❌ Need 'Points for Objectives' row."))
            st.stop()
        ...
        m2.markdown(f"**{t('🏫 Class:')}** {meta.get('Class', 'N/A')}")
        m3.markdown(f"**{t('📅 Date:')}** {meta.get('Date', 'N/A')}")
        m4.markdown(f"**{t('📝 Assessment:')}** {meta.get('Assessment name', 'N/A')}")

        st.markdown(
            f"### 📝 {t('Name:')} **{meta.get('Assessment name', 'N/A')}** "
            f"| 📚 {t('Subject:')} **{meta.get('Subject', 'N/A')}**"
        )
        ...
        mask = (
            raw_students[first_column].astype(str).str.strip().str.lower()
            == "points for objectives"
        )
        ...
        student_df = raw_students[~mask].copy().dropna(subset=[first_column])
        student_df = student_df.rename(columns={first_column: "Student Name"})
        ...
        for column in obj_names:
            student_df[column] = pd.to_numeric(student_df[column], errors="coerce").fillna(0)

        total_max = sum(obj_max)

        st.info(f"📋 {t('Auto Total Max Mark')} = **{total_max:g}**")

        st.markdown(f"### 📚 {t('Objectives')}")

        for index, obj in enumerate(obj_names, 1):
            st.markdown(f"{index}. **{obj}** – {obj_desc.get(obj, obj)}")
        ...
        for _, row in student_df.iterrows():
            ...
            for index, column in enumerate(obj_names):
                ...
                if value > obj_max[index]:
                    errors.append(f"• {row['Student Name']}: {column}={value} > max {obj_max[index]}")

                if value < 0:
                    errors.append(f"• {row['Student Name']}: {column} Negative")
        ...
        if errors:
            ...
            if st.button(t("Analyze Assessment"), key="analyze_assessment_button"):
                results = []
                ...
                for _, row in student_df.iterrows():
                    ...
                    results.append({
                        ...
                        "Total %": round(total_percentage, 1),
                        "Level": level
                    })
                ...
                counts = rdf["Level"].value_counts().to_dict()

                c1, c2, c3, c4, c5, c6 = st.columns(6)
                c1.metric(t("Absent"), counts.get(t("Absent"), 0))
                c2.metric(t("Fail"), counts.get(t("Fail"), 0))
                c3.metric(t("Acceptable"), counts.get(t("Acceptable"), 0))
                c4.metric(t("Good"), counts.get(t("Good"), 0))
                c5.metric(t("Very Good"), counts.get(t("Very Good"), 0))
                c6.metric(t("Outstanding"), counts.get(t("Outstanding"), 0))

                valid_percentages = rdf["Total %"].dropna()
                total_students = len(valid_percentages)

                if total_students > 0:
                    percentage_60_or_more = (valid_percentages >= 60).sum() / total_students * 100
                    percentage_above_60 = (valid_percentages > 60).sum() / total_students * 100
                    percentage_above_75 = (valid_percentages > 75).sum() / total_students * 100
                else:
                ...
                st.success(f"**{overall}** ({t('Max')} {total_max:g})")

                level_df = rdf["Level"].value_counts().reset_index()
                level_df.columns = ["Level", "Count"]

                ordered_levels = [t(level) for level in ORDER]
                level_df["Level"] = pd.Categorical(
                    level_df["Level"], categories=ordered_levels, ordered=True
                )
                ...
                with v1:
                    ...
                    chart_df = rdf.dropna(subset=["Total %"])
                    fig = px.bar(chart_df, x="Student Name", y="Total %", color="Level", range_y=[0, 100])
                    st.plotly_chart(fig, use_container_width=True)

                with v2:
                    st.subheader(t("📊 Level Distribution"))
                    pie_fig = px.pie(
                        level_df, names="Level", values="Count", color="Level",
                        color_discrete_map={t(key): value for key, value in COLORS.items()},
                        hole=0.3
                    )
                ...
                support_chart_df = rdf.dropna(subset=["Total %"])
                support_fig = px.bar(
                    support_chart_df, x="Student Name", y="Total %", color="Support Level", range_y=[0, 100]
                )
                st.plotly_chart(support_fig, use_container_width=True)

                support_count = rdf["Support Level"].value_counts().reset_index()
                support_count.columns = [t("Support Level"), t("Students")]
                ...
                rdf.to_excel(excel_buffer, index=False)
...
# =========================================================
    ...
    n_assessments = st.number_input(
        t("🔢 Number of assessments"), min_value=2, max_value=10, value=2, step=1,
        key="number_of_assessments"
    )

    assessment_files = []

    for index in range(int(n_assessments)):
        assessment_files.append(
            st.file_uploader(
                f"📄 {t('Assessment')} {index + 1}", type=["xlsx", "xls"],
    ...
    if all(assessment_files):
        ...
        for index, file in enumerate(assessment_files):
            meta, df = read_objectives_file(file)

            if meta is None or df is None:
                st.error(t("❌ File missing 'Points for Objectives' row."))
                st.stop()

            metadata_list.append(meta)

            percentage_column = f"Pct{index + 1}"
            keep = df[["Student Name", "Pct"]].rename(columns={"Pct": percentage_column})

            if merged is None:
                merged = keep
            else:
                merged = pd.merge(merged, keep, on="Student Name", how="outer")
        ...
        for index, metadata in enumerate(metadata_list):
            st.markdown(
                f"**File {index + 1}: {metadata.get('Assessment name', 'N/A')}** | "
        ...
        merged[percentage_columns] = merged[percentage_columns].fillna(0)

        merged["Difference"] = (
            merged[percentage_columns[-1]] - merged[percentage_columns[0]]
        ).round(1)

        merged["Status"] = merged["Difference"].apply(
            lambda difference: t("Growth") if difference > 0.5
            else t("Decay") if difference < -0.5
            else t("Same")
        )

        merged["Support Level"] = merged[percentage_columns[-1]].apply(support_level)
        ...
        growth_count = status_counts.get(t("Growth"), 0)
        decay_count = status_counts.get(t("Decay"), 0)
        same_count = status_counts.get(t("Same"), 0)

        st.subheader(t("📢 Summary"))

        m1, m2, m3 = st.columns(3)
        m1.metric(f"🟩 {t('Growth')}", growth_count)
        m2.metric(f"🟥 {t('Decay')}", decay_count)
        m3.metric(f"🟨 {t('Same')}", same_count)

        status_df = pd.DataFrame({
            "Status": [t("Growth"), t("Decay"), t("Same")],
            "Count": [growth_count, decay_count, same_count]
        })
        ...
        with v1:
            ...
            bar_fig = px.bar(
                ...
                color_discrete_map={t("Growth"): "green", t("Decay"): "red", t("Same"): "yellow"}
            )
        ...
        with v2:
            ...
            pie_fig = px.pie(
                status_df, names="Status", values="Count", color="Status",
                color_discrete_map={t("Growth"): "green", t("Decay"): "red", t("Same"): "yellow"},
                hole=0.3
            )
            pie_fig.update_traces(textinfo="percent+label")
        ...
        average_data = merged[percentage_columns].mean().reset_index()
        average_data.columns = ["Assessment", "Average"]
        average_data["Assessment"] = average_data["Assessment"].str.replace("Pct", "Assess", regex=False)

        st.subheader(t("📈 Average Score Trend (%)"))
        average_fig = px.line(average_data, x="Assessment", y="Average", markers=True)
        st.plotly_chart(average_fig, use_container_width=True)

        st.subheader(t("📈 Student Growth (Difference)"))
        growth_fig = px.bar(merged, x="Student Name", y="Difference", color="Status")
        st.plotly_chart(growth_fig, use_container_width=True)

        support_count = merged["Support Level"].value_counts().reset_index()
        support_count.columns = [t("Support Level"), t("Students")]
        ...
        merged.to_excel(comparison_buffer, index=False)
...
# =========================================================
    ...
    map_file = st.file_uploader(t("📄 Upload MAP Data Excel"), type=["xlsx", "xls"], key="map_upload")
    ...
    if map_file:
        try:
            ...
            if missing_columns:
                st.error(t("❌ Missing columns: ") + ", ".join(missing_columns))
                st.stop()

            for column in ["Previous RIT", "Current RIT", "Percentile"]:
            ...
            map_df["Growth Status"] = map_df["RIT Growth"].apply(
                lambda value: t("Growth") if value > 0
                else t("Decay") if value < 0
                else t("Same")
            )

            map_df["Support Level"] = map_df["Percentile"].apply(support_level)
            ...
            average_previous = map_df["Previous RIT"].mean()
            average_current = map_df["Current RIT"].mean()
            average_growth = map_df["RIT Growth"].mean()
            average_percentile = map_df["Percentile"].mean()

            c1, c2, c3, c4 = st.columns(4)
            c1.metric(t("👥 Students"), total_students)
            c2.metric(t("📉 Previous Avg RIT"), round(average_previous, 1))
            c3.metric(t("📈 Current Avg RIT"), round(average_current, 1))
            c4.metric(t("🚀 Average Growth"), round(average_growth, 1))

            st.metric(t("🎯 Average Percentile"), round(average_percentile, 1))

            st.markdown("---")

            status_count = map_df["Growth Status"].value_counts().reset_index()
            ...
            with v1:
                ...
                growth_fig = px.bar(map_df, x="Student Name", y="RIT Growth", color="Growth Status")
                st.plotly_chart(growth_fig, use_container_width=True)

            with v2:
                st.subheader(t("📊 Growth Distribution"))
                growth_pie = px.pie(status_count, names="Status", values="Count", hole=0.3)
            ...
            percentile_fig = px.bar(
                map_df, x="Student Name", y="Percentile", color="Support Level", range_y=[0, 100]
            )
            st.plotly_chart(percentile_fig, use_container_width=True)

            st.subheader(t("👥 Support Groups"))

            support_count = map_df["Support Level"].value_counts().reset_index()
            support_count.columns = [t("Support Level"), t("Students")]
            st.dataframe(support_count, use_container_width=True)
            ...
            map_df.to_excel(map_buffer, index=False)
            ...
            st.error(t("❌ Error reading MAP file: ") + str(error))
...
# =========================================================
    ...
    gaps_file = st.file_uploader(t("📄 Upload Single Sheet"), type=["xlsx", "xls"], key="gaps_upload")

    if gaps_file:
        metadata, gaps_df = read_gaps_file(gaps_file)

        if metadata is None or gaps_df is None:
            st.error(t("❌ File missing required rows/columns."))
            st.stop()

        st.subheader(t("📋 Assessment Information"))

        st.markdown(
            f"**{t('👩‍🏫 Teacher:')}** {metadata.get('Teacher Name', 'N/A')} | "
            f"**{t('🏫 Class:')}** {metadata.get('Class', 'N/A')} | "
            f"**{t('📅 Date:')}** {metadata.get('Date', 'N/A')} | "
            f"**{t('📝 Assessment:')}** {metadata.get('Assessment name', 'N/A')} | "
            f"**{t('📚 Subject:')}** {metadata.get('Subject', 'N/A')}"
        )

        gaps_df["Difference"] = (gaps_df["Pct2"] - gaps_df["Pct1"]).round(1)

        gaps_df["Status"] = gaps_df["Difference"].apply(
            lambda difference: t("Growth") if difference > 0.5
            else t("Decay") if difference < -0.5
            else t("Same")
        )

        gaps_df["Support Level"] = gaps_df["Pct2"].apply(support_level)

        st.subheader(t("📊 Comparison Table (Percentage Based)"))
        st.dataframe(gaps_df.style.map(color_cell, subset=["Status"]), use_container_width=True)

        counts = gaps_df["Status"].value_counts().to_dict()
        growth_count = counts.get(t("Growth"), 0)
        decay_count = counts.get(t("Decay"), 0)
        same_count = counts.get(t("Same"), 0)

        st.subheader(t("📢 Summary"))

        mc1, mc2, mc3 = st.columns(3)
        mc1.metric(f"🟩 {t('Growth')}", growth_count)
        mc2.metric(f"🟥 {t('Decay')}", decay_count)
        mc3.metric(f"🟨 {t('Same')}", same_count)
        ...
        gap_fig = px.bar(gaps_df, x="Student Name", y="Difference", color="Status")
        st.plotly_chart(gap_fig, use_container_width=True)

        support_count = gaps_df["Support Level"].value_counts().reset_index()
        support_count.columns = [t("Support Level"), t("Students")]
        ...
        comparison_buffer = io.BytesIO()
        gaps_df.to_excel(comparison_buffer, index=False)
...
# =========================================================
    ...
    if service == t("Compare between sections"):
        ...
        # OBJECTIVES
        if comparison_type == t("By Assessment Objectives"):
            number_of_classes = st.number_input(
                t("Number of classes"), min_value=2, max_value=10, value=2, step=1,
                key="number_of_classes"
            )

            section_files = []

            for index in range(int(number_of_classes)):
                section_files.append(
                    st.file_uploader(
                        f"📄 {t('Class')} {index + 1} {t('file')}",
                        type=["xlsx", "xls"],
            ...
            if all(section_files):
                ...
                for index, file in enumerate(section_files, 1):
                    (metadata, section_df, objective_names,
                     objective_max, objective_descriptions) = read_section_file(file)

                    if metadata is None:
                        st.error(t("❌ Class file invalid"))
                        st.stop()

                    class_name = metadata.get("Class", f"{t('Class')} {index}")

                    st.markdown(f"### 📋 {t('Class')} {index} {t('Info')}")

                    m1, m2, m3, m4 = st.columns(4)
                    m1.markdown(f"**{t('👩‍🏫 Teacher:')}** {metadata.get('Teacher Name', 'N/A')}")
                    m2.markdown(f"**{t('🏫 Class:')}** {class_name}")
                    m3.markdown(f"**{t('📅 Date:')}** {metadata.get('Date', 'N/A')}")
                    m4.markdown(f"**{t('📝 Assessment:')}** {metadata.get('Assessment name', 'N/A')}")
                    ...
                    def calculate_band(percentage):
                        ...
                        if percentage <= 85:
                            return t("76-85% (Very Good)")
                        return t("86-100% (Excellent)")

                    section_df["Band"] = section_df["Pct"].apply(calculate_band)

                    objective_average = {}

                    for objective in objective_names:
                        maximum = objective_max.get(objective, 0)

                        if maximum > 0:
                            objective_average[objective] = (
                                section_df[objective] / maximum * 100
                            ).mean()
                        else:
                            objective_average[objective] = 0

                    sections_data.append({
                        "name": class_name,
                        "df": section_df,
                        "bands": section_df["Band"].value_counts(),
                        "obj_avg": objective_average,
                        "obj_names": objective_names,
                        "obj_desc": objective_descriptions
                    })
                ...
                for section in sections_data:
                    counts = section["bands"].reindex(band_order).fillna(0).astype(int)

                    row = {t("Class"): section["name"]}
                    for band in band_order:
                        row[band] = counts.get(band, 0)
                ...
                plot_df = band_df.melt(
                    id_vars=t("Class"), value_vars=band_order,
                    var_name=t("Band"), value_name=t("Count")
                )

                st.subheader(t("📊 Band Distribution per Class"))

                band_fig = px.bar(
                    plot_df, x=t("Band"), y=t("Count"), color=t("Class"),
                    barmode="group", text=t("Count")
                )
        ...
        # TOTAL MARK
        elif comparison_type == t("By Assessment Total Mark"):
            st.info(t("This comparison option is ready for total-mark based analysis."))

        # EXTERNAL BENCHMARK
        elif comparison_type == t("By External Benchmark Assessment"):
            st.info(t("This comparison option is ready for external benchmark analysis."))
